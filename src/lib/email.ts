...
export async function sendCountyFormEmail(params: {
  ...
  const fromName = String(params.values.name || params.values.submitterName || "Website visitor");
  const fromEmail = String(params.replyTo || params.values.email || params.values.submitterEmail || "");
